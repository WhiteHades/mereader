...
df["retrieval_effectiveness_log"] = np.log10(
    df["retrieval_effectiveness"].clip(lower=1e-6)
)
...
print(
    f"[INFO] Loaded {len(df)} total rows ({len(regular_df)} regular, {len(spoiler_df)} spoiler)"
)
...
def create_precision_recall_plane():
    ...
    model_means = (
        regular_df.groupby("model")
        .agg({"bert_precision": "mean", "bert_recall": "mean", "bert_f1": "mean"})
        .reset_index()
    )
    ...
    model_color_map = {
        model: discrete_colors[i % len(discrete_colors)]
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            range=[0.5, 0.8], title=dict(font=dict(size=32)), tickfont=dict(size=24)
        ),
        yaxis=dict(
            range=[0.4, 0.8], title=dict(font=dict(size=32)), tickfont=dict(size=24)
        ),
        legend=dict(font=dict(size=24), x=0.02, y=0.98),
...
def create_speed_quality_efficiency():
    ...
    model_means = (
        regular_df.groupby("model")
        .agg({"response_time": "mean", "bert_f1": "mean", "context_count": "mean"})
        .reset_index()
    )
    ...
    sorted_models = model_means.sort_values(
        ["response_tire", "bert_f1"], ascending=[True, False]
    )
    frontier_models = []
    best_quality = -np.inf
    ...
    fig.update_layout(
        ...
        legend=dict(font=dict(size=24), x=0.02, y=0.10),
...
def create_reading_progress_robustness():
    ...
    progress_data = (
        regular_df.groupby(["model", "progress_stage"])
        .agg({"bert_precision": "mean", "bert_recall": "mean", "bert_f1": "mean"})
        .reset_index()
    )
    ...
    if not use_progress:
        ...
        try:
            ...
            ctx_bin = pd.qcut(
                tmp["context_count"].rank(method="first"),
                q=3,
                labels=["low", "mid", "high"],
                duplicates="drop",
            )
        ...
        if hasattr(ctx_bin, "cat") and len(ctx_bin.cat.categories) >= 2:
            ...
            tmp["context_bin"] = pd.Series(
                ["all"] * len(tmp), index=tmp.index, dtype="category"
            )

        progress_data = (
            tmp.groupby(["model", "context_bin"])
            .agg({"bert_precision": "mean", "bert_recall": "mean", "bert_f1": "mean"})
            .reset_index()
            .rename(columns={"context_bin": "progress_stage"})
        )
    ...
    for i, model in enumerate(models):
        model_data = progress_data[progress_data["model"] == model].sort_values(
            "progress_stage"
        )
    ...
    fig.update_layout(
        ...
        width=1500,
        template="plotly_white",
        legend=dict(font=dict(size=24), x=1.02, y=0.5),
    )
...
def create_error_profile_analysis():
    ...
    error_counts = (
        filtered_df.groupby(["model", "error_type"]).size().unstack(fill_value=0)
    )
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=(
            "Error Type Distribution by Model (% Share)",
            "Error Count by Model and Type",
        ),
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        xaxis=dict(
            tickangle=45, title=dict(font=dict(size=28)), tickfont=dict(size=22)
        ),
        xaxis2=dict(
            tickangle=45, title=dict(font=dict(size=28)), tickfont=dict(size=22)
        ),
...
def create_holistic_quality_radar():
    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(visible=True, range=[0, 1], tickfont=dict(size=22)),
            angularaxis=dict(tickfont=dict(size=24)),
        ),
        width=800,
        height=800,
        template="plotly_white",
        legend=dict(font=dict(size=24), x=0.85, y=-0.15),
    )
